...
window.onload=function ()
{
	...
	for(i=1;i<=NUM;i++)
	{
		...
		oImg.src='assets/admin/files/img/abs/'+i+'.jpg';
	}
	...
	for(i=1;i<=NUM;i++)
	{
		...
		oImg.src='assets/admin/files/img/abs/thumbs/'+i+'.jpg';
	}
...
function loadedSucc()
{
	...
	for(var j=0;j<ROW;j++)
	{
		for(var i=0;i<COL;i++,k++)
		{
			...
			oDiv.wc_bg='url(assets/admin/files/img/abs/thumbs/'+k+'.jpg)';
			oDiv.wc_row=j;
			oDiv.wc_col=i;
			...
			aDiv[ii].onclick=function ()
			{
				...
				else
				{
					(function (){
						...
						{
							var oSpan=aDiv[i].getElementsByTagName('span')[0];
							oSpan.style.background='url(assets/admin/files/img/abs/'+_this.index+'.jpg) '+-aDiv[i].wc_col*dw+'px '+-aDiv[i].wc_row*dh+'px';
	...
	oPrev.onclick=oNext.onclick=function ()
	{
		...
		var timer=setInterval(function (){
			...
			aDiv[item].getElementsByTagName('span')[0].style.background='url(assets/admin/files/img/abs/'+(iNow+1)+'.jpg) '+-aDiv[item].wc_col*dw+'px '+-aDiv[item].wc_row*dh+'px';
